...
class HospitalRAGPipeline:
    ...
    def _run_ai_fallback(self, query: str) -> dict:
        ...
        providers = [
            ("ollama", f"Ollama ({self.ollama_service.model})", self.ollama_service.generate),
            ("groq", f"Groq ({self.groq_service.model})", self.groq_service.generate),
            ("huggingface", f"HuggingFace ({self.huggingface_service.model})", self.huggingface_service.generate),
        ]

        for key, label, fn in providers:
            try:
                text = fn(prompt)
                model_outputs[key] = {"label": label, "ok": True, "answer": text}
                if primary_text is None and text:
                    primary_text = text
            except Exception as exc:
                model_outputs[key] = {"label": label, "ok": False, "error": str(exc)}

        if primary_text is None:
            primary_text = "No AI provider returned a response."
